        ...
        try (CompactionController controller = getCompactionController(transaction.originals()))
        {
                 ...
                 CompactionIterator ci = new CompactionIterator(compactionType, scanners.scanners, controller, nowInSec, taskId))
            {
                ...
                try (CompactionAwareWriter writer = getCompactionAwareWriter(cfs, getDirectories(), transaction, actuallyCompact))
                {
                    ...
                    while (ci.hasNext())
                    {
                        ...
                        //StorageService.instance.compactionWriteTime += System.currentTimeMillis() - appendStartTime;
            ...
            else
            {
                ...
                logger.debug(String.format("Compacted (%s) %d sstables to [%s] to level=%d.  %s to %s (~%d%% of original) in %,dms.  Read Throughput = %s, Write Throughput = %s, Row Throughput = ~%,d/s.  %,d total partitions merged to %,d.  Partition merge counts were {%s}",
                                           ...
                                           totalKeysWritten,
                                           mergeSummary));
                logger.debug("######insertMemTable:{} s, insertLog:{} s, flushMemTable:{} s, compaction:{} s, compactionWriteTime:{} s", StorageService.instance.insertMemTable/1000, StorageService.instance.insertLog/1000, StorageService.instance.flushMemTable/1000, StorageService.instance.compaction/1000, StorageService.instance.compactionWriteTime/1000);
                ...
                StorageService.instance.totoalCompactionNum++;
                float compactSizethisTime = (float)endsize/(float)1048576;
                float compactTime = (System.currentTimeMillis() - startTime)/1000;
                logger.debug("----cfs.name:{}, compactTime:{} s, compactSizethisTime:{} MB, comptThpt:{} MB/s", cfs.name, compactTime, compactSizethisTime, compactSizethisTime/compactTime);       
                logger.debug("----totoalCompactionNum:{}",  StorageService.instance.totoalCompactionNum);
