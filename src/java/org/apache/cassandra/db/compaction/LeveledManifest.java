...
public class LeveledManifest
{
    ...
    public synchronized CompactionCandidate getCompactionCandidates()
    {
        ...
        //for (int i = generations.length - 1; i > 0; i--)
        {
            ...
            logger.debug("Compaction score for level {} is {}, maxSize:{}", i, score, (double)maxBytesForLevel(i, maxSSTableSizeInBytes));
            //if (score > 1.001)
            if (score > 1.001 || i==0)
            {
                ...
                */
                ...
                if (!candidates.isEmpty())
                {
                    /*int nextLevel = 0;
                    if(i==0) nextLevel = 1;
                    else nextLevel = getNextLevel(candidates);*/
    ...
    public void printEachLevelSize(){
        ...
        List<SSTableReader> sstables1 = getLevel(0);
        StorageService.instance.totalSizeOfL0 = SSTableReader.getTotalBytes(sstables1)/1024;

        logger.debug("######SSTableNumOfL0:{}", StorageService.instance.SSTableNumOfL0);
        ...
        logger.debug("######insertMemTable:{} s, insertLog:{} s, flushMemTable:{} s, compaction:{} s", StorageService.instance.insertMemTable/1000, StorageService.instance.insertLog/1000, StorageService.instance.flushMemTable/1000, StorageService.instance.compaction/1000);
    ...
    public int getNextLevel(Collection<SSTableReader> sstables)
    {
        ...
        {
            logger.debug("######maxSSTableSizeInBytes: {}, SSTableReader.getTotalBytes: {}", maxSSTableSizeInBytes, SSTableReader.getTotalBytes(sstables));
            newLevel = 0;
        }
        else
        {
